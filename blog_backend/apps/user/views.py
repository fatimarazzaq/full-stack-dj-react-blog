...
def account_activate(request, uidb64, token):
    try:
        ...
        return Response({'message': 'Your email has been verified. You need to login.'}, status=status.HTTP_200_OK)
...
def activateEmail(request, user, to_email):
    ...
    verification_url = f"{settings.SITE_URL}{reverse('activate', kwargs={'uidb64': urlsafe_base64_encode(force_bytes(user.pk)), 'token': account_activation_token.make_token(user)})}"
